...
| `Initiatives/<kebab-slug>.md` | One note per goal-directed workstream (`type: initiative`, lifecycle `status:`); built from `Initiatives/Initiative TEMPLATE.md`, run via `Jobs/Run an initiative.md`. Done initiatives move to `Initiatives/archive/`. **Structural folder — not the inbox.** |
...
| `Writing/<Outlet>/` | *(optional)* Writing pipelines — one subfolder per publication outlet (stage flow inside, e.g. IDEAS→DRAFTS→PUBLISH + RESOURCES); runbook lives in `Jobs/`. **Structural folder — not the inbox.** |
| `Excalidraw/*.excalidraw.md` | *(optional)* Visual diagrams (Obsidian Excalidraw plugin) generated by `meta/bin/excalidraw.py` — see `Jobs/Create an Excalidraw diagram.md`. **Structural folder — not the inbox.** |
...
`concepts/`, `Initiatives/`, `meta/`, `raw/`, `daily/`, `People/`, `Jobs/`, `attachments/`, `docs/`, `Writing/` (optional), and `Excalidraw/` (optional) are **structural folders — not inbox items**.
...
- Root inbox clean: only the four anchors + structural folders (`concepts/`, `Initiatives/`, `meta/`, `raw/`, `daily/`, `People/`, `Jobs/`, `attachments/`, `docs/`, `Writing/`, `Excalidraw/`).
