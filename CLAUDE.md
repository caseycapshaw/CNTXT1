...
| `Initiatives/<kebab-slug>.md` | One note per goal-directed workstream (`type: initiative`, lifecycle `status:`); built from `Initiatives/Initiative TEMPLATE.md`, run via `Skills/run-an-initiative/SKILL.md`. Done initiatives move to `Initiatives/archive/`. **Structural folder — not the inbox.** |
...
| `Skills/<kebab-slug>/SKILL.md` | One agent-invocable runbook per recurring "job to be done" (Claude Code Agent Skills format) — single source of truth for the *steps*; built from `Skills/SKILL TEMPLATE.md`, indexed by `concepts/skills.md`, symlinked from `.claude/skills/<slug>` for invocation. **Structural folder — not the inbox.** |
| `Writing/<Outlet>/` | *(optional)* Writing pipelines — one subfolder per publication outlet (stage flow inside, e.g. IDEAS→DRAFTS→PUBLISH + RESOURCES); skill lives in `Skills/`. **Structural folder — not the inbox.** |
| `Excalidraw/*.excalidraw.md` | *(optional)* Visual diagrams (Obsidian Excalidraw plugin) generated by `meta/bin/excalidraw.py` — see `Skills/create-an-excalidraw-diagram/SKILL.md`. **Structural folder — not the inbox.** |
...
- **Skills:** every recurring task = one agent-invocable `SKILL.md` runbook in `Skills/<slug>/` (the single source of truth for the *steps*, Claude Code Agent Skills format), built from `Skills/SKILL TEMPLATE.md`, symlinked from `.claude/skills/<slug>` for invocation (natural language or `/<slug>`). `concepts/skills.md` is the **index**. Steps live in the skill; the *why/context* stays in the matching concept.
...
- **Initiatives:** a goal-directed workstream (multiple actions over time) = one note in `Initiatives/` with `type: initiative` frontmatter and lifecycle `status: active|paused|done`. Live ones sit on the index Quick map's Initiatives line; actions stay inline in the initiative note. Concept↔initiative conversions are a `git mv` + frontmatter swap + link-map regen. Skill: `Skills/run-an-initiative/SKILL.md`.
- **Wikilink resolution:** `meta/link-map.md` maps every `[[target]]` (concept slug, People name+alias, Skill slug+alias) → its file path. Use it instead of grepping; regenerate with `meta/bin/build-link-map.sh` after adding/renaming any of those.
...
`concepts/`, `Initiatives/`, `meta/`, `raw/`, `daily/`, `People/`, `Skills/`, `attachments/`, `docs/`, `Writing/` (optional), and `Excalidraw/` (optional) are **structural folders — not inbox items**.
...
`Skills/sync-an-improvement-to-cntxt1/SKILL.md` — always re-templated to
...
updates *inward* (`Skills/pull-framework-updates-from-cntxt1/SKILL.md`); author
...
- Root inbox clean: only the four anchors + structural folders (`concepts/`, `Initiatives/`, `meta/`, `raw/`, `daily/`, `People/`, `Skills/`, `attachments/`, `docs/`, `Writing/`, `Excalidraw/`).
- No `[[wikilink]]` points to a non-existent note (including `[[Full Name]]` person-links → a real `People/` note or registered alias).
- Every concept and initiative listed in `index.md`; every `Skills/` skill listed in `concepts/skills.md`.
