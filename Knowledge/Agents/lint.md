...
> _Generated from `.claude/agents/lint.md` by `SYSTEM/bin/build_claude_mirrors.py` — edit the canonical file, never this mirror. Generated: 2026-08-27_
...
Steps:
1. Run `SYSTEM/bin/lint.sh` from the vault root and capture its output in
   full. This covers the mechanical checks: root inbox clean, all wikilinks
   resolve, index completeness, frontmatter validates against the schemas in
   `SYSTEM/schemas/`.
...
4. If you were explicitly told to fix specific, narrow issues (e.g. "resolve
   ...
   `SYSTEM/bin/lint.sh` to confirm green before reporting.
