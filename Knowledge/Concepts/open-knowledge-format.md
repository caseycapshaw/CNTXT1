...
A vault built on this starter kit (see [[karpathy-method]] / [[SCHEMA]]) is
functionally a **superset** of OKF — nearly every OKF idea already exists:

| OKF | CNTXT1 vault |
| :-- | :-- |
| `type` required frontmatter | ✓ `type: concept/initiative` + Knowledge/People/Skills frontmatter |
...
| Per-directory `index.md` | Functional equivalent: [[contacts]] indexes `Knowledge/People/`, [[skills]] indexes `Knowledge/Skills/` |
| `log.md` | ✓ `SYSTEM/log.md` (append-only one-liners vs OKF's newest-first groups — dialect difference) |
| Markdown path links | **Divergent:** Obsidian `[[wikilinks]]` + `SYSTEM/link-map.md` |
| Citations / provenance | ✓ stronger: every concept fact must trace to a `Knowledge/raw/` capture |
| Bundle = distribution unit | A private vault ships nothing — conformance matters only for what you *export* or *ingest* |

Strict conformance would require frontmatter on `Knowledge/raw/`/`daily/` files and
...
spec is silent (lint, link-map, Knowledge/raw/compiled separation, Actions). Converting
...
1. **Inbound:** when someone shares an OKF bundle, ingest it like any source —
   clone, walk its indexes, compile durable facts into `Knowledge/Concepts/` with raw
...
- [[SCHEMA]] — this kit's schema (where the `description:` rule lives).
