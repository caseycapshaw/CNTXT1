...
description: Audits the always-loaded surfaces for stale state-shaped claims and fixes drift in place. Use when monthly with the optimize pass, when the index Quick map feels behind reality, or on request.
metadata:
  title: Audit State Freshness
  type: check
  domain: kb-meta
