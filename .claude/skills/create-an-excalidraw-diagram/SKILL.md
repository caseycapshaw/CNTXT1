...
description: Produces an editable Excalidraw diagram in Knowledge/Excalidraw/, embedded in its home note. Use when you need a diagram / flowchart / visual map in the vault.
metadata:
  title: Create an Excalidraw Diagram
  type: format
  domain: kb-meta
  trigger: you need a diagram / flowchart / visual map in the vault
  frequency: ad-hoc
  tools: ["Excalidraw (Obsidian plugin)", "python3 (stdlib only)", "mermaid"]
  owner: "{{NAME}}"
  status: active
  tags: [format, kb-meta]
  aliases: ["Create an Excalidraw diagram", "Create Excalidraw", "Make an Excalidraw diagram", "Excalidraw diagram", "create-an-excalidraw-diagram"]
  summary: Produces an editable Excalidraw diagram in Knowledge/Excalidraw/, embedded in its home note.
---
...
> **Outcome:** a file in `Knowledge/Excalidraw/` that opens in **Excalidraw view** as real shapes + arrows (not raw text, no duplicated text pile), labels centred in boxes, arrows that re-route when a box is dragged.
...
### Route A — `SYSTEM/bin/excalidraw.py` (self-contained, no external deps)
1. **Describe the diagram as a spec** in `SYSTEM/bin/excalidraw.py`. Add a function returning `(name, Diagram)` and append it to `DIAGRAMS`. The builder API:
   - `d = Diagram("Title …")`
...
2. **Generate:** `python3 SYSTEM/bin/excalidraw.py --all` (or `python3 SYSTEM/bin/excalidraw.py "Overview"` for one; `--list` to list). Files land in `Knowledge/Excalidraw/`.
...
4. **Hand-refine** and save into `Knowledge/Excalidraw/`. The AI gives the first draft; you do the layout pass.
...
- **`Knowledge/Excalidraw/` is a registered structural folder** (`SYSTEM/bin/lint.sh` + `SYSTEM/SCHEMA.md` + `CLAUDE.md`) — new drawings there don't trip the inbox lint. Reference them with plain links, not `[[wikilinks]]` (the lint resolver only knows Knowledge/Concepts/Knowledge/Initiatives/Knowledge/People/Skills).
- **This is an optional extension** (see `SYSTEM/SCHEMA.md` § Optional extensions) — nothing else in the KB depends on it. Skip it entirely if you don't need visual diagrams.
...
- [ ] File lives in `Knowledge/Excalidraw/`; `SYSTEM/bin/lint.sh` is green.
