...
description: Fans a task out into N worker agents running in split CMUX panes, each driven through the four-verb loop and torn down when finished. Use when one task decomposes into concurrent sub-tasks you want to run and watch as panes.
metadata:
  title: Spawn Subagent Panes in a CMUX Workspace
  type: do
  domain: kb-meta
  ...
  status: active
  tags: [do, kb-meta]
  aliases: ["Spawn subagent panes in a CMUX workspace", "Spawn subagent panes", "Fan out subagents", "Subagent panes", "Spawn workers in panes", "spawn-subagent-panes-in-a-cmux-workspace"]
  summary: Fans a task out into N worker agents running in split CMUX panes, each driven through the four-verb loop and torn down when finished.
---
