---
name: audit-the-kb-system
description: Full-system architectural audit — parallel auditors over structure/UX, interlinking, consistency, and connected systems, producing a ranked report + phased fix plan executed phase-by-phase. Use when you ask for a high-level / full-system / architectural audit of the KB and its connected systems.
metadata:
  title: Audit the KB System
  type: check
  domain: kb-meta
...
2. **Fan out 4 parallel read-only subagents**, one per dimension (each returns raw
   ...
   - **Structure/UX** — folder inventory + misplacements; `index.md` quality (measure
     ...
     this); redundancy/drift across `CLAUDE.md` / `SYSTEM/SCHEMA.md` / `index.md`;
...
[[SCHEMA]] (the schema being audited) · [[karpathy-method]] (why lint is
