...
description: Produces a green/issues verdict on KB mechanical health (broken links, stale actions, inbox pile-up). Use when on request, or automatically at end of day if automated.
metadata:
  title: Run the KB Health Check
  type: check
  domain: kb-meta
