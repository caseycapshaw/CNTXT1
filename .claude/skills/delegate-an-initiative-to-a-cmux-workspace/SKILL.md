...
description: Hands a whole task/initiative to its own CMUX workspace (own repo/worktree, own agent) kicked off against the shared Knowledge/Initiatives/ note. Use when an initiative/task is ready to run in its own repo context, delegated from the main workspace.
metadata:
  title: Delegate an Initiative to a CMUX Workspace
  type: do
  domain: kb-meta
  ...
  status: active
  tags: [do, kb-meta]
  aliases: ["Delegate an initiative to a CMUX workspace", "Delegate to a workspace", "Spin up a CMUX workspace", "Delegate an initiative", "delegate-an-initiative-to-a-cmux-workspace"]
  summary: Hands a whole task/initiative to its own CMUX workspace (own repo/worktree, own agent) kicked off against the shared Knowledge/Initiatives/ note.
---
...
> Code on the right `--cwd`, kicked off against an `Knowledge/Initiatives/<slug>.md` note
...
**optional extension** — see `SYSTEM/SCHEMA.md` § Optional extensions for the
...
in `Knowledge/Initiatives/` (if not, run [[Run an initiative]] first) — that note is the
...
5. **Launch the delegated Claude Code** into the lead surface (pick the model by
   ...
   append `Knowledge/Agents/initiative-worker.md` (absolute path — the
   ...
   ```bash
   cmux send --surface "$LEAD" "claude --model <model-id> --strict-mcp-config --mcp-config '{\"mcpServers\":{}}' --append-system-prompt <vault>/Knowledge/Agents/initiative-worker.md"
...
6. **Kick it off with a one-line brief** that points at the vault note as its
   ...
   ```bash
   cmux send --surface "$LEAD" "Read <vault>/Knowledge/Initiatives/<slug>.md; execute its Now-and-next; write decisions/actions back to that note; end with: DONE: <slug> | <summary>"
   cmux send-key --surface "$LEAD" enter
   ```
7. **Record the dispatch in the vault** *(this is the round-trip — do not skip)* —
   in `Knowledge/Initiatives/<slug>.md` add a dated Milestone ("delegated to CMUX workspace
...
- **State lives in the vault, not the workspace.** A CMUX workspace is disposable;
  if the plan/result isn't written into `Knowledge/Initiatives/<slug>.md`, it's lost on
...
- [ ] The dispatch (and, at close, the outcome) is written back into `Knowledge/Initiatives/<slug>.md`.
- [ ] The `DONE:` sentinel was confirmed via `read-screen` (not assumed from a notification).

## Related
- `SYSTEM/SCHEMA.md` § Optional extensions — the model + why (the two-interface split, the round-trip rule, `Knowledge/Agents/`).
- [[Spawn subagent panes in a CMUX workspace]] — sibling runbook for pane-level fan-out.
- [[Run an initiative]] — creates/maintains the `Knowledge/Initiatives/` note this delegates against.
