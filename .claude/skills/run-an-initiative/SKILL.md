...
description: 'Opens, updates, or closes a Knowledge/Initiatives/ note that carries a multi-action workstream from start to done. Use when an outcome needs multiple actions over time (rule of thumb: 3+ actions or more than a week).'
metadata:
  title: Run an Initiative
  type: do
  domain: kb-meta
