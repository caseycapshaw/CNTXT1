...
`Skills/sync-an-improvement-to-cntxt1/SKILL.md`, which ends with a personal-identifier
...
- **Never push from this vault to the public CNTXT1 repo** — no pushable
  ...
  via `Skills/pull-framework-updates-from-cntxt1/SKILL.md`. Author generic
...
- **Backlinks:** use Obsidian-style `[[wikilinks]]` to connect notes. Reference people by `[[Full Name]]` (their `People/` note); nicknames resolve via the note's `aliases:` frontmatter field. A `[[link]]` to a note that doesn't exist yet is fine — it marks something worth writing later. Cite raw captures as path-style wikilinks — `[[raw/YYYY-MM-DD-topic]]` (no `.md`) — so citations hyperlink in Obsidian; backticks are only for filename *patterns*/templates, never for a reference to a real file.
...
- **`meta/link-map.md` resolves wikilinks in one lookup.** A generated table mapping every `[[target]]` (concept slug, People name + `aliases:`, Skill slug + `aliases:`) → its file path — read it instead of grepping. **Regenerate after adding/renaming any concept, person, or skill:** `meta/bin/build-link-map.sh` (idempotent).
- **Raw notes** are append-only and dated; they preserve the original source.
- **Root is the inbox, not a home for permanent files.** Only `README.md`, `index.md`, `Actions.md`, and `CLAUDE.md` live there permanently; everything else at the root is transient and should be triaged into `raw/` (then compiled). Permanent machinery lives in `meta/`. Structural folders (`concepts/`, `Initiatives/`, `meta/`, `raw/`, `daily/`, `People/`, `Skills/`, `attachments/`, `docs/`, `Writing/` [optional], `Excalidraw/` [optional]) are **not inbox items**.
...
- `index.md` lists every concept and initiative file; `concepts/skills.md` lists every skill in `Skills/`.
- **Inbox is empty:** the vault root holds only `README.md` + `index.md` + `Actions.md` + `CLAUDE.md` (plus the structural folders: `concepts/`, `Initiatives/`, `meta/`, `raw/`, `daily/`, `People/`, `Skills/`, `attachments/`, `docs/`, `Writing/` [optional], `Excalidraw/` [optional]). Any other root file is un-triaged — file it into `raw/` and compile. If some automation writes to a fixed root file (a machinery write-target, not an inbox item), register it in `lint.sh`'s `standing` list — triage its *contents*, keep the file.
...
**Daily-note rollup:** if you adopt daily notes, `meta/bin/rollup-daily-weeks.sh`
...
`Excalidraw/` structural folder. See `Skills/create-an-excalidraw-diagram/SKILL.md`.
...
own repo context and its own agent — `Skills/delegate-an-initiative-to-a-cmux-workspace/SKILL.md`) and pane-level (one task fans out into concurrent sub-tasks
sharing a context — `Skills/spawn-subagent-panes-in-a-cmux-workspace/SKILL.md`). Both
...
**Domain-advisor roles** extend the same `meta/agent-roles/` idea beyond
...
file built from `meta/agent-roles/domain-advisor TEMPLATE.md` (scope, data
sources, operating principles, output shape) paired with one on-demand `Skills/`
skill built from `Skills/Run a domain review TEMPLATE.md` that loads the role and walks
