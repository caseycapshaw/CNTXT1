...
- **Inbox is empty:** the vault root holds only `README.md` + `index.md` + `Actions.md` + `CLAUDE.md` (plus the structural folders: `concepts/`, `Initiatives/`, `meta/`, `raw/`, `daily/`, `People/`, `Jobs/`, `attachments/`, `docs/`, `Writing/` [optional], `Excalidraw/` [optional]). Any other root file is un-triaged — file it into `raw/` and compile. If some automation writes to a fixed root file (a machinery write-target, not an inbox item), register it in `lint.sh`'s `standing` list — triage its *contents*, keep the file.
...
structural folder (registered in `lint.sh`); the pipeline's runbook lives in
`Jobs/` like any other job.
...
sources, operating principles, output shape) paired with one on-demand `Jobs/`
runbook built from `Jobs/Run a domain review.md` that loads the role and walks
a review. The role carries the *stance* (what to watch, how to reason about
the domain); the job carries the *steps*; findings land back in the domain's
