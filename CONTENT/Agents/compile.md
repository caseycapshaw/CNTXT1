...
`CONTENT/Concepts/`, following the vault's own compiler discipline exactly.

Before writing anything:
1. Read `SYSTEM/AGENTS.md` in full — the schema for this KB (frontmatter
   format, wikilink conventions, CONTENT/People/CONTENT/Skills/Initiatives
...
Rules:
- **Never delete or rewrite a `CONTENT/raw/` capture** — it's append-only
...
- If the fact is really about a named person or a recurring task, it likely
  belongs in `CONTENT/People/<Full Name>.md` or
  `CONTENT/Skills/<TYPE>/<Imperative Title>.md` instead of a concept — check
  `SYSTEM/AGENTS.md` before deciding where it lands.
