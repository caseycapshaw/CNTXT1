...
| OKF | CNTXT1 vault |
| :-- | :-- |
| `type` required frontmatter | ✓ `type: concept/initiative` + CONTENT/People/Skills frontmatter |
...
| Per-directory `index.md` | Functional equivalent: [[contacts]] indexes `CONTENT/People/`, [[skills]] indexes `CONTENT/Skills/` |
...
| Citations / provenance | ✓ stronger: every concept fact must trace to a `CONTENT/raw/` capture |
| Bundle = distribution unit | A private vault ships nothing — conformance matters only for what you *export* or *ingest* |

Strict conformance would require frontmatter on `CONTENT/raw/`/`daily/` files and
...
spec is silent (lint, link-map, CONTENT/raw/compiled separation, Actions). Converting
...
1. **Inbound:** when someone shares an OKF bundle, ingest it like any source —
   clone, walk its indexes, compile durable facts into `CONTENT/Concepts/` with raw
