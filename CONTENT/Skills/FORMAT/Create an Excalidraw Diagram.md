...
summary: Produces an editable Excalidraw diagram in CONTENT/Excalidraw/, embedded in its home note.
---
...
> **Outcome:** a file in `CONTENT/Excalidraw/` that opens in **Excalidraw view** as real shapes + arrows (not raw text, no duplicated text pile), labels centred in boxes, arrows that re-route when a box is dragged.
...
2. **Generate:** `python3 SYSTEM/bin/excalidraw.py --all` (or `python3 SYSTEM/bin/excalidraw.py "Overview"` for one; `--list` to list). Files land in `CONTENT/Excalidraw/`.
...
4. **Hand-refine** and save into `CONTENT/Excalidraw/`. The AI gives the first draft; you do the layout pass.
...
- **`CONTENT/Excalidraw/` is a registered structural folder** (`SYSTEM/bin/lint.sh` + `SYSTEM/AGENTS.md` + `CLAUDE.md`) — new drawings there don't trip the inbox lint. Reference them with plain links, not `[[wikilinks]]` (the lint resolver only knows CONTENT/Concepts/CONTENT/Initiatives/CONTENT/People/Skills).
- **This is an optional extension** (see `SYSTEM/AGENTS.md` § Optional extensions) — nothing else in the KB depends on it. Skip it entirely if you don't need visual diagrams.
...
- [ ] File lives in `CONTENT/Excalidraw/`; `SYSTEM/bin/lint.sh` is green.
