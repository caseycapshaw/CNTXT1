...
summary: Hands a whole task/initiative to its own CMUX workspace (own repo/worktree, own agent) kicked off against the shared CONTENT/Initiatives/ note.
---
...
> Code on the right `--cwd`, kicked off against an `CONTENT/Initiatives/<slug>.md` note
...
**optional extension** — see `SYSTEM/AGENTS.md` § Optional extensions for the
...
in `CONTENT/Initiatives/` (if not, run [[Run an initiative]] first) — that note is the
...
6. **Kick it off with a one-line brief** that points at the vault note as its
   ...
   ```bash
   cmux send --surface "$LEAD" "Read <vault>/CONTENT/Initiatives/<slug>.md; execute its Now-and-next; write decisions/actions back to that note; end with: DONE: <slug> | <summary>"
   cmux send-key --surface "$LEAD" enter
   ```
7. **Record the dispatch in the vault** *(this is the round-trip — do not skip)* —
   in `CONTENT/Initiatives/<slug>.md` add a dated Milestone ("delegated to CMUX workspace
...
- **State lives in the vault, not the workspace.** A CMUX workspace is disposable;
  if the plan/result isn't written into `CONTENT/Initiatives/<slug>.md`, it's lost on
...
- [ ] The dispatch (and, at close, the outcome) is written back into `CONTENT/Initiatives/<slug>.md`.
