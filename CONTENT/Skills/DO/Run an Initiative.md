...
summary: Opens, updates, or closes a CONTENT/Initiatives/ note that carries a multi-action workstream from start to done.
---
...
> `CONTENT/Initiatives/` that carries the whole workstream from open to done
...
1. Copy `CONTENT/Initiatives/Initiative TEMPLATE.md` → `CONTENT/Initiatives/<kebab-slug>.md`.
...
- A concept that turns out to be goal-shaped converts by `git mv` into
  `CONTENT/Initiatives/` + frontmatter swap to `type: initiative` + the missing
  sections + link-map regen (wikilinks are basename-based, so existing links
  keep resolving).

## Done when

- [ ] Initiative note exists in `CONTENT/Initiatives/` with `type: initiative` frontmatter and a two-line Outcome.
