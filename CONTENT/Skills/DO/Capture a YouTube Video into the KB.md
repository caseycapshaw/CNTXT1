...
summary: Captures a YouTube video's metadata, description links, and transcript digest into CONTENT/raw/ and compiles it into the relevant concept(s)/person(s).
---

# Skill — Capture a YouTube video into the KB

> **When:** {{NAME}} shares a YouTube link to capture · **Frequency:** ad-hoc · **Tools:** `yt-dlp`
> **Outcome:** a dated `CONTENT/raw/` capture of the video (metadata + description links + transcript digest), compiled into the concept(s)/person(s) named, indexed and logged. **Every note created must link back to the video and carry the links from its description.**
...
default to one `CONTENT/raw/` capture + one concept; add a `CONTENT/People/` note when the
...
5. **Raw capture → `CONTENT/raw/YYYY-MM-DD-<topic>.md`** with a provenance header:
...
6. **Compile** per the requested target shape:
   - **Concept:** new or updated `CONTENT/Concepts/<slug>.md` (frontmatter per [[AGENTS]]) distilling the durable ideas — with a **Source** line linking the video and the relevant description links.
...
- [ ] `CONTENT/raw/` capture exists with video URL + description links + condensed transcript
