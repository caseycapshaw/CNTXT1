...
<!-- TEMPLATE — copy to CONTENT/Skills/DO/Run a {{Domain}} Review.md and fill every
     {{placeholder}}. Pairs with a role file built from
     `CONTENT/Agents/domain-advisor TEMPLATE.md`: the role carries the
...
1. **Load the role:** read `CONTENT/Agents/{{domain}}-advisor.md` —
...
4. **KB sync:** execute the role's KB-sync contract (CONTENT/raw/ capture → home-note
...
- [ ] KB-sync contract executed for a substantive pull (CONTENT/raw/, home note, index, log)

## Related

`{{home note}}` · `CONTENT/Agents/{{domain}}-advisor.md` (the role) · [[skills]] · [[AGENTS]]
