...
3. **Review — route learnings by scope.** From the session (corrections {{NAME}} made rank highest, then step-1 feedback, then observed friction): a learning that binds *one role* → propose an edit to that `CONTENT/Agents/<role>.md` (show the diff; bump its version on adoption). One that binds *a skill* → same, in the skill. One that binds *the system* (`SYSTEM/AGENTS.md`, templates, `CLAUDE.md`) → **draft, never apply** — present for {{NAME}}'s yes or park it in the relevant note's Open questions. "No friction = no findings" — the door costs nothing extra.
...
[[AGENTS]] · [[Run the KB Health Check]] · [[Keep Machinery Vendor-Portable]] · `SYSTEM/decisions.md`.
