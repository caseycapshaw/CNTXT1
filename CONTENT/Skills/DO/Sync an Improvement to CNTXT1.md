...
**Personal content never leaves this vault** — see `SYSTEM/AGENTS.md` § Privacy
...
anyone running the CNTXT1 method — a schema tweak in `SYSTEM/AGENTS.md`, a
new/improved `CONTENT/Skills/` skill, a `SYSTEM/bin/` script fix, a template change.
...
1. **Classify the changed file(s):**
   - **Copy verbatim** (no personal content ever lives here): `setup.md`,
     `SYSTEM/AGENTS.md`, `SYSTEM/bin/*` scripts, `CONTENT/Concepts/karpathy-method.md`,
     `CONTENT/Skills/Skill TEMPLATE.md`, `CONTENT/People/People TEMPLATE.md`,
     `CONTENT/Initiatives/Initiative TEMPLATE.md`, `SYSTEM/optional/automation/*`
     (already parameterized with `{{VAULT}}` / `{{NAME}}`).
   - **Copy after checking frontmatter:** other `CONTENT/Skills/<TYPE>/*.md`
   ...
   - **Copy only the skeleton, re-templated** — never the live rows/content:
     `CLAUDE.md`, `index.md`, `README.md`, `CONTENT/Concepts/contacts.md`,
     `CONTENT/Concepts/skills.md`. Diff for what changed *structurally* (a new
     ...
     version over it.
   - **Never copy:** anything in `CONTENT/People/` (except the template), personal
     concepts and initiatives, `CONTENT/raw/`, `daily/`, `Actions.md`, `SYSTEM/log.md`,
...
`SYSTEM/AGENTS.md` § Privacy & content separation (the rule) · [[karpathy-method]] · [[AGENTS]]
