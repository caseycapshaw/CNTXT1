...
Raw notes live in `CONTENT/raw/`, are named `YYYY-MM-DD-topic.md`, and are **append-only**
...
"source code" the LLM compiles into `CONTENT/Concepts/`.
...
- Durable facts from this note get **compiled** into `CONTENT/Concepts/`, then the new concept is added to `index.md` and a line is appended to `SYSTEM/log.md`.

## Actions surface inline, next to their context
- [ ] Delete this example file after writing my first real capture #action
- [ ] Read `CONTENT/Concepts/karpathy-method.md` to understand why this works #action #priority
