...
register in `~/Library/Application Support/Claude/claude_desktop_config.json`:

    {
      "mcpServers": {
        "cntxt": {
          "command": "python3",
          "args": ["/absolute/path/to/your-kb/SYSTEM/bin/kb-mcp-server.py"]
        }
      }
    }

Vault root is resolved from this file's location (SYSTEM/bin/ → vault), so the
...
LINK_MAP = VAULT / "SYSTEM" / "link-map.md"
...
def tool_kb_capture(args):
    ...
    with (VAULT / "SYSTEM" / "log.md").open("a", encoding="utf-8") as f:
        f.write(
