...
Steps:
1. Run `SYSTEM/bin/lint.sh` from the vault root and capture its output in full.
   This covers the mechanical checks: root inbox clean, all wikilinks resolve,
   index completeness, frontmatter present.
2. Read `SYSTEM/AGENTS.md`'s "Health checks" section for the **judgment** checks
   ...
   an `CONTENT/Initiatives/` note whose `updated:` is stale relative to its actions.
   Do a pass over the notes you were pointed at (or, if none were named, over
   recently-touched files — check `SYSTEM/log.md`'s tail for what changed
