...
- **Rename/convention-change hygiene (anti-drift):** a rename or convention change isn't done when the primary artifact moves — it's done when **no prose still describes the old world**. Last step of any migration: grep the whole vault (including `CONTENT/Skills/` runbooks, READMEs, and this file) for the old name/pattern and fix every hit; keep old names resolvable via `aliases:`; regenerate the link map. Prefer **referencing** a single source (a script, a generated table) over copying its contents into prose — every copy is a future stale fact.
