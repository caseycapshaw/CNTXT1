...
- **Vendor portability:** every enforced guarantee lives at a vendor-independent tier (script/ritual or OS-scheduled job); a harness surface (hook, skill shim) may *accelerate* it but never solely carry it, and **a script failure is declared, never imitated** by model judgment. Full rule: `CONTENT/Skills/RULE/Keep Machinery Vendor-Portable.md`.
